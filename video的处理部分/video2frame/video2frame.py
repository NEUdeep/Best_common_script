...
VIDEO_PATH = '/Users/kanghaidong/Desktop/video(9).MP4'  # 视频地址
EXTRACT_FOLDER = '/Users/kanghaidong/Desktop/22'  # 存放帧图片的位置
EXTRACT_FREQUENCY = 10  # 帧提取频率 Frames per Second(FPS)
...
def extract_frames(video_path, dst_folder, index):
    ...
    while True:
        ...
        if count % EXTRACT_FREQUENCY == 0:
            save_path = "{}/cs4_{:>03d}.jpg".format(dst_folder, index)
...
def extract(video_path, dst_folder, index):
    import cv2
    ...
    while True:
        ...
        if frame is None:
            break
        if count % EXTRACT_FREQUENCY == 0:
            save_path = "{}highway-smoke_{:>03d}.jpg".format(dst_folder, index)
            cv2.imwrite(save_path, frame)
            index += 1
        count += 1
    video.release()
